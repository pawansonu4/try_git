...
about="This calculator is created at W3"
...
def result():
    ...
    try:
        ...
        if storeMemory == int(storeMemory):
            storeMemory = int(storeMemory)
        displayText.set((str(storeMemory)))
        return
    except:
        displayText.set("Math error")
        return
...
# gui elements
display = Entry(app, state=DISABLED, justify=RIGHT, bd=2, disabledbackground="pale green",
                textvariable=displayText).grid(row=0, column=0, columnspan=4, ipadx=5, ipady=5, padx=5, pady=5)
btnClear = Button(app, text="C", command=lambda: clear(), bg="indian red", fg="white").grid(row=1, column=0, ipadx=5)
btnDivide = Button(app, text="/", command=lambda: click("/"), bg="light blue", fg="white").grid(row=1, column=1,
                                                                                                ipadx=5)
btnMultiply = Button(app, text="*", command=lambda: click("*"), bg="light blue", fg="white").grid(row=1, column=2,
                                                                                                  ipadx=5)
btnSubtract = Button(app, text="-", command=lambda: click("-"), bg="light blue", fg="white").grid(row=1, column=3,
                                                                                                  ipadx=5)
...
btnAdd = Button(app, text="+", command=lambda: click("+"), bg="light blue", fg="white", height=1, width=1).grid(row=2,
                                                                                                                column=3,
                                                                                                                rowspan=2,
                                                                                                                ipadx=5,
                                                                                                                ipady=14)
...
btnEquals = Button(app, text="=", command=lambda: click("="), bg="light blue", fg="white", height=1, width=1).grid(
    row=4, column=3, rowspan=2, ipadx=5, ipady=14)
